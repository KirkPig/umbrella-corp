...
func _card_exit(_exit_global_pos: Vector2):
	...
	for i in n:
		...
		card_in_play[i].change_card_position(_exit_pos, 0.2)
		for j in range(i + 1, n):
			card_in_play[j].change_card_position(_old_pos_list[j - 1], 0.2)
...
func _card_enter(_nodes: Array[Card]):
	...
	for i in n:
		...
		for _node in _nodes:
			_old_pos_list.append(_node.position)
		_nodes[i].change_card_position(Vector2(field_width / 3 * 2, 0), 0.2)
		for j in range(i + 1, n):
			_nodes[j].change_card_position(_old_pos_list[j - 1], 0.2)
